...
import uz.fido.pfexchange.entity.minyust.MinyustFamilyInf;

@Repository
public interface MinyustFamilyInfRepository
    extends JpaRepository<MinyustFamilyInf, Long> {
    @Query(
        value = """
        Select Mfi.Pinpp
    ...
    @Query(
        value = """
        Select Count(*)
          From Pf_Exchange_Minyust_Family t
         Where t.Pinpp = :pinpp
           And t.Status = '06'
           """,
        nativeQuery = true
    )
    Integer minyustCount(@Param("pinpp") String pinpp);
}
